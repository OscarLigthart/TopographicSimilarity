...
from .generate_dataset import generate_dataset, one_hot
...
class CompositionalDataset(Dataset):
    """
    Referential Game Dataset that holds compositional messages
    This dataset is used to train a receiver on compositional language
    """
    ...
    def __getitem__(self, indices):
        ...
        # based on target, we get a message
        # get the indices of the ones in the target
        message = np.where(np.array(target) == 1)[0]

        # convert to one hot vector of length 20 get LENGTH
        message = one_hot(message, len(target))

        # return
        return (message, target, distractors)
...
def get_referential_dataloader(
    ...
    dataset = CompositionalDataset(data)
    return DataLoader(
        dataset,
        pin_memory=True,
